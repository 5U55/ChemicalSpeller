function getElementTypes() {
    ...
    for (var i = 0; i < elements.length; i++) {
        if (elementTypes.includes(elements[i].category) == false) {
            elementTypes.push(elements[i].category)
        }
    }
    console.log(elementTypes)
}

var elementCategororyColors = {
    "white": "white",
    "diatomic nonmetal": "salmon",
    "noble gas": "gold",
    "alkali metal": "springgreen",
    "alkaline earth metal": "chartreuse",
    "metalloid": "turquoise",
    "polyatomic nonmetal": "orange",
    "post-transition metal": "lightskyblue",
    "transition metal": "hotpink",
    "lanthanide": "mediumslateblue",
    "actinide": "violet",
    "unknown, probably transition metal": "#89E0FF",
    "unknown, probably post-transition metal": "#FFB6DB",
    "unknown, probably metalloid": "#93DBD4",
    "unknown, predicted to be noble gas": "#FFF18E",
    "unknown, but predicted to be an alkali metal": "#83FDC0",
    "amino acid": "gray",
    "unit of measurement": "darkgray"
}

function spinSpell(){
  for(var i=0;i<document.getElementById("words").value.length; i++){
    if(document.getElementById("words").value[i].match(/[a-z]/i) || document.getElementById("words").value[i] == " "){
  } else {
    $("#error").html("can only spell with letters")
    return;
  }
  }
   $("#error").html("")
  $('#spin').show()
    setTimeout(function(){spellWord();$('#spin').hide()}, 10);
}

document.getElementById("words").addEventListener("keyup", function(event) {
  if (event.keyCode === 13) {
    event.preventDefault();
    document.getElementById("spellButton").click();
  }
});

function spellWord() {
    var wordsToSpell = document.getElementById("words").value.toLowerCase();
    var arr = []
    arr.push(new Array(wordsToSpell.length));
    for (var j = 0; j < wordsToSpell.length; j++) {
        for (var i = 0; i < elements.length; i++) {
            if (elements[i].symbol.length == 1) {
                if (wordsToSpell[j].includes(elements[i].symbol.toLowerCase())) {
                    arr[0][j] = elements[i].symbol;
                }
            }
        }
    }
    for (var j = 0; j < wordsToSpell.length; j++) {
        for (var i = 0; i < elements.length; i++) {
            if (elements[i].symbol.length == 2) {
                var string = wordsToSpell[j] + wordsToSpell[j + 1]
                if (string == null) {
                    continue;
                }
                if ((string.toLowerCase() == elements[i].symbol.toLowerCase())) {
                    var stringify = JSON.stringify(arr[0]);
                    var newarr = JSON.parse(stringify);
                    newarr[j] = elements[i].symbol;
                    newarr[1 + j] = elements[i].symbol;
                    arr.push(newarr);
                }
            }
        }
        ...
        for (var k = 0; k < elements.length; k++) {
            for (var j = 0; j < arr[i].length; j++) {
              if(arr[j] != undefined && arr[j+1] != undefined){
                if (elements[k].symbol.length == 2) {
                    var string = arr[i][j] + arr[i][j + 1];
                    if (string.toLowerCase() == elements[k].symbol.toLowerCase()) {
                        var stringify = JSON.stringify(arr[i]);
                        var newarr = JSON.parse(stringify);
                        newarr[j] = elements[k].symbol;
                        newarr[1 + j] = elements[k].symbol;
                        if (arr.includes(newarr) == false) {
                            arr.push(newarr);
                        }
                    }
                }
    ...
    for (var j = 0; j < finalArr.length; j++) {
        for (var i = 0; i < finalArr[j].length; i++) {
          if(finalArr[j][i]!= undefined){
            if (finalArr[j][i] == finalArr[j][i + 1] && finalArr[j][i].length == 2) {
                finalArr[j].splice(i, 1);
            }}
        }
    }
    console.log(finalArr);
    var lowestCount = wordsToSpell.length;
    var cleanestArr = [];
    for (var k = 0; k < wordsToSpell.length; k++) {
        for (var j = 0; j < finalArr.length; j++) {
            var thisCount = 0;
            for (var i = 0; i < finalArr[j].length; i++) {
                if (finalArr[j][i] != undefined) {
                    if (finalArr[j][i] == finalArr[j][i].toLowerCase()) {
                        thisCount++;
                    }
                }
            }
            if (thisCount == k) {
                cleanestArr.push(finalArr[j]);
            }
        }
        if (cleanestArr.length > 1) {
            console.log(cleanestArr)
            break;
        }
        if (k == wordsToSpell.length - 1) {
            cleanestArr = finalArr;
            console.log(cleanestArr)
        }
    }
    var shortest = finalArr[0];
    if (document.getElementById("shortest").checked == true) {
        for (var j = 0; j < cleanestArr.length; j++) {
            if (shortest.length > cleanestArr[j].length) {
                console.log(cleanestArr[j].length);
                shortest = cleanestArr[j];
            }
        }
    } else {
        shortest = cleanestArr[0];
    }
   ...
   var allTypes = [];
    for (var i = 0; i < shortest.length; i++) {
        if (shortest[i] == null) {
            ...
            console.log("nullSkip");
        }
        for (var j = 0; j < elements.length; j++) {
            set(shortest, allTypes, i, j)
        }
    }
    if (document.getElementById("showAll").checked == true) {
      ...
      var list = "";
        for (var k= 0; k < finalArr.length; k++) {
          if (list.includes(finalArr[k].toString()) == false) {
          for (var i = 0; i < finalArr[k].length; i++) {
            for (var j = 0; j < elements.length; j++) {
          set(finalArr[k], allTypes, i, j)
            }}list+=finalArr[k]
            document.getElementById("result").innerHTML += "<br>"
          }
  ...
  document.getElementById("colorCodes").innerHTML = "";
  if (document.getElementById("colors").checked == true) {
        for (var k = 0; k < allTypes.length; k++) {
          if($("#onlyElems").prop("checked") == true){
            if(allTypes[k]!= "amino acid" && allTypes[k] != "unit of measurement"){
              document.getElementById("colorCodes").innerHTML +=  "<span class='colorSwatches' style='background-color:" +
                    elementCategororyColors[allTypes[k]] + "'></span> : "+ allTypes[k]+"<br>"
            }
          } else {
            document.getElementById("colorCodes").innerHTML +=  "<span class='colorSwatches' style='background-color:" +
                    elementCategororyColors[allTypes[k]] + "'></span> : "+ allTypes[k]+"<br>"
    ...
    if($("#showMass").prop("checked") == false){
        $(".mass").hide();
    }
}

function set(shortest, allTypes, i, j){
  var catColor;
  var emp = " "
            if (document.getElementById("colors").checked == true) {
                catColor = elements[j].category;
            } else {
                catColor = "white";
            }
            if (shortest[i] != null) {
                if (elements[j].symbol == shortest[i].toString()) {
                    if (allTypes.includes(catColor) == false) {
                        allTypes.push(catColor);
                    }
                      if($("#onlyElems").prop("checked") == true && shortest[i] == shortest[i].toLowerCase()){
                       
                        document.getElementById("result").innerHTML +=
                         "<div class='outerBox' style='background-color:" +
                        elementCategororyColors["white"] + "'><div class='atomicNumber'>" + elements[j].number + "</div><div class='atomicSymbol'>" +
                        elements[j].symbol + "</div><div class='mass' style='color:white'>-</div><div class='elementName' style='color:white'> - </div></div>";
                      
                    } else{
                    document.getElementById("result").innerHTML +=
                        "<div class='outerBox' style='background-color:" +
                        elementCategororyColors[catColor] + "'><div class='atomicNumber'>" + elements[j].number + "</div><div class='atomicSymbol'>" +
                        elements[j].symbol + "</div><div class='mass'>"+elements[j].atomic_mass+"</div><div class='elementName'>" + elements[j].name + "</div></div>";
                }}
            }
}
